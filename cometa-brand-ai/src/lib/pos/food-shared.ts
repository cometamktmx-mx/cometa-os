...
export type FoodCheck = {
  id: string; table_id: string | null; status: "OPEN" | "PAYMENT_PENDING" | "CLOSED";
...
export type FoodProduct = { id: string; product_name: string; name: string; category: string; price: number; tax_rate: number; available: number | null; image_url?: string | null; modifier_groups?: FoodModifierGroup[] };
...
export function foodTableState(check: FoodCheck | undefined, tickets: readonly FoodTicket[]): FoodTableState {
  if (!check || check.status === "CLOSED") return "AVAILABLE";
