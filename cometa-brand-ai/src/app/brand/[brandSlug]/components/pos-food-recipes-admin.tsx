...
function Workspace({ brandSlug, location, locationId, kind, optionId }: { brandSlug: string; location?: Location; locationId: string; kind: 'inventory' | 'products' | 'effects'; optionId?: string }) {
  ...
  const detail = catalog?.ingredients.find(item => item.id === detailId);
  const activeIngredients = catalog?.ingredients.filter(i => i.active) || [];
  return <>
    <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-cyan-300">Food · Administración</p><h1 className="mt-2 text-3xl font-bold">{kind === 'inventory' ? 'Insumos y existencias' : kind === 'products' ? 'Menú · Preparado con receta' : 'Impacto en receta'}</h1><p className="mt-2 text-sm text-slate-400">{kind === 'products' ? 'Su disponibilidad se calcula automáticamente según tus insumos.' : 'Controla insumos por sucursal, presentaciones de compra y costos.'}</p></div><div className="flex gap-2"><button className={button} disabled={busy} onClick={() => void run(async () => { await load(); }, false)}>Actualizar disponibilidad</button><Link className={button} href={`/brand/${brandSlug}/pos/admin`}>Administración</Link>{kind !== 'effects' && <button disabled={busy || !catalog} className={button + ' bg-cyan-300 text-slate-950'} onClick={() => { if (kind === 'inventory') setIngredient(emptyIngredient()); else { setProduct(null); setNewProduct(true); setComponents([]); setProductForm({ name: '', description: '', price: 0, tax_rate: 0, active: true, category_id: '' }); setProductImageUrl(''); setOriginalProductImageUrl(''); setImageError(null); } }}>{kind === 'inventory' ? 'Crear insumo' : 'Crear preparado'}</button>}</div></div>
    ...
    {catalog && kind === 'inventory' && <>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{catalog.ingredients.filter(i => i.name.toLocaleLowerCase().includes(search.toLocaleLowerCase())).map(i => <article className={panel} key={i.id}><div className="flex justify-between"><div><p className="text-xs text-cyan-300">{categories[i.category]}</p><h2 className="mt-1 text-lg font-semibold">{i.name}</h2></div><span className="text-xs text-slate-400">{!i.active ? 'Inactivo' : i.stock - i.reserved_stock <= 0 ? 'Agotado' : i.stock - i.reserved_stock <= i.minimum_stock ? 'Pocas existencias' : 'Disponible'}</span></div><p className="mt-5 text-3xl font-bold">{i.stock} <span className="text-sm text-slate-400">{unitLabel(i.unit_code)}</span></p><p className="mt-1 text-xs text-slate-400">Físico · stock mínimo {i.minimum_stock} · reservado {i.reserved_stock}{i.presentations.find(p => p.active) ? ` · equivalente ${formatEquivalent(i)}` : ""}</p><p className="mt-2 text-sm text-emerald-200">Utilizable estimado: {Number(i.usable_estimated_stock).toFixed(3)} {i.unit_code}</p><p className="mt-3 text-sm">Costo: {money(i.unit_cost, 6)} / {unitLabel(i.unit_code)}</p><p className="mt-2 text-xs text-slate-400">Compra: {i.presentations.filter(p => p.active).map(p => p.name).join(' · ') || 'Sin presentación configurada'}</p><p className="mt-1 text-xs text-slate-400">Merma estimada {i.waste_percent}% · {i.supplier_name || 'Sin proveedor'}</p><div className="mt-4 flex flex-wrap gap-2"><button className={button} onClick={() => setDetailId(i.id)}>Ver insumo</button><button className={button} disabled={busy || !i.active} onClick={() => setIngredient({ id: i.id, name: i.name, category: i.category, unit_code: i.unit_code, initial_quantity: 0, minimum_quantity: i.minimum_stock, waste_percent: i.waste_percent, supplier_name: i.supplier_name || '', active: i.active, presentations: i.presentations.map(p => ({ id: p.id, name: p.name, content: p.conversion_factor, unit_code: p.base_unit_code, cost: p.configured_cost || 0, supplier_name: p.supplier_name || '', active: p.active })) })}>Editar</button>{(['receive', 'adjust', 'cost_set'] as const).map(action => <button key={action} className={button} disabled={busy || !i.active} onClick={() => setOperation({ ingredient: i, action, quantity: action === 'adjust' ? 0 : 1, content: i.presentations.find(p => p.active)?.conversion_factor || 1, cost: i.presentations.find(p => p.active)?.configured_cost || 0, unit_code: i.unit_code, presentation_id: i.presentations.find(p => p.active)?.id || '', notes: '', request_key: crypto.randomUUID() })}>{action === 'receive' ? 'Entrada' : action === 'adjust' ? 'Ajustar' : 'Costo'}</button>)}{!i.active && <button className={button} disabled={busy} onClick={() => void run(async () => { await save('ingredient_status', { id: i.id, active: true }); await load(); })}>Reactivar</button>}</div></article>)}</div>
    ...
    {catalog && kind === 'products' && <><label className="block max-w-lg text-xs text-slate-400">Buscar producto<input className={field} value={search} onChange={e => setSearch(e.target.value)} /></label><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{catalog.products.filter(p => p.name.toLocaleLowerCase().includes(search.toLocaleLowerCase())).map(p => <button disabled={busy} key={p.id} className={panel + ' text-left'} onClick={() => editProduct(p)}><p className="text-xs text-cyan-300">{p.inventory_mode === 'recipe' ? `Receta ${p.recipe ? 'v' + p.recipe.version : 'pendiente'}` : 'Inventario ' + p.inventory_mode}</p><h2 className="mt-2 text-lg font-semibold">{p.name} <small className="text-slate-400">{p.variant_name}</small></h2><p className="mt-4 text-2xl font-bold">{money(p.price)}</p><p className="mt-2 text-sm text-slate-400">{p.recipe ? `Costo ${money(p.recipe.unit_cost)}` : 'Configura o revisa su receta'}</p><div className="mt-3 rounded-xl border border-white/10 p-3"><strong className={p.recipe?.availability === 0 ? 'text-rose-200' : 'text-cyan-200'}>{!p.active ? 'Inactivo' : p.recipe?.availability == null ? 'Receta pendiente' : p.recipe.availability < 1 ? 'Agotado' : p.recipe.availability <= 1 || p.recipe.components.some(c => c.available_stock <= (catalog.ingredients.find(i => i.id === c.ingredient_variant_id)?.minimum_stock || 0)) ? 'Pocas existencias' : 'Disponible'}</strong><p className="mt-1 text-sm">{p.recipe?.availability == null ? 'Publica la receta para calcular porciones.' : `${p.recipe.availability} porciones disponibles`}</p>{p.recipe?.limiting_ingredient && <p className="mt-1 text-xs text-slate-400">Limitante: {p.recipe.limiting_ingredient}</p>}</div></button>)}</div>{!catalog.products.length && <p className={panel}>Crea tu primer preparado y agrega su receta.</p>}</>}
