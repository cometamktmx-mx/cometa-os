...
function accountReference(check: FoodCheck) {
  return check.order_number != null ? String(check.order_number) : check.id.replace(/-/g, "").slice(-6).toUpperCase();
}
...
export function PosFoodOperations({ operator, brandSlug, branding }: { operator: PosOperator; brandSlug: string; branding?: PosBranding | null }) {
  ...
  const tableLabel = (value: FoodCheck) => data?.tables.find(table => table.id === value.table_id)?.name || `${value.service_type === "TAKEAWAY" ? "Para llevar" : "Mostrador"} #${accountReference(value)}`;
  ...
  const canVoid = staffHasAnyRole(operator, ["ADMIN", "MANAGER"]);
  ...
  return <section aria-label="Operación Food" className={floor ? "grid gap-6 lg:grid-cols-[190px_minmax(0,1fr)] xl:gap-8" : "space-y-5"}>
    ...
    {view === "salon" && !check && data?.location && <section className={panel}><h2 className="text-lg font-bold">Nueva orden</h2><p className="mt-1 text-sm">Mesa: selecciona una mesa del sal?n.</p><div className="mt-3 flex gap-3"><button className={primary} disabled={busy} onClick={() => { setCustomer(''); setOpening('COUNTER'); }}>Mostrador</button><button className={button} disabled={busy} onClick={() => { setCustomer(''); setOpening('TAKEAWAY'); }}>Para llevar</button></div></section>}
    {view === "salon" && !check && data && <section className="grid gap-3 md:grid-cols-3">{(['Preparando','Listos','Entregados'] as const).map(stage => <div className={panel} key={stage}><h2 className="font-bold">{stage}</h2>{data.checks.filter(order => order.service_type === 'COUNTER' || order.service_type === 'TAKEAWAY').filter(order => { const tickets = data.tickets.filter(t => t.check_id === order.id); const status = tickets.length && tickets.every(t => t.served_at) ? 'Entregados' : tickets.some(t => t.status === 'READY' && !t.served_at) ? 'Listos' : 'Preparando'; return status === stage; }).map(order => <button type="button" className={button + ' mt-3 block w-full text-left'} key={order.id} onClick={() => setSelected(order.id)}><strong>{tableLabel(order)}</strong><span className="block text-sm">{order.customer_name || 'Sin nombre'} ? {elapsedFoodTime(order.opened_at,now)}</span></button>)}</div>)}</section>}
    ...
    {!data ? <div role="status" className={`${panel} py-20 text-center`}>{error ? "Operación sin sincronizar. Reintenta para continuar." : "Cargando mesas y comandas…"}</div> : !data.location ? <div className={`${panel} py-20 text-center`}>{data.locations.length ? "Selecciona una sucursal para operar." : "No hay una sucursal activa. Revisa la configuración de POS."}</div> : <>
      {view === "kitchen" ? <><span className="sr-only">Sin comandas pendientes</span><KitchenBoard data={data} now={now} busy={busy} tableLabel={tableLabel} staffName={staffName} onAction={(action, ticket) => void act(action, { checkId: ticket.check_id, ticketId: ticket.id })} /></> : check ? <>
         ...
         <div className={`grid items-start gap-5 ${canEdit ? "xl:grid-cols-[minmax(0,1fr)_340px] 2xl:grid-cols-[minmax(0,1fr)_380px]" : pendingPayment || closed ? "lg:grid-cols-1" : "lg:grid-cols-2"}`}>
          ...
          <div className={canEdit ? "space-y-4 xl:sticky xl:top-36" : "space-y-4"}>
            ...
            {checkTickets.map(ticket => <div key={ticket.id} className={`${panel} ${ticket.status === "READY" && !ticket.served_at ? "ring-2 ring-emerald-400" : ""}`}><div className="flex justify-between gap-3"><h3 className="font-bold">Envío {ticket.sequence}</h3><span className="font-semibold">{ticket.served_at ? "Entregado" : ticket.status === "READY" ? "Listo para entregar" : ticket.status === "PREPARING" ? "Preparando" : "En cocina"}</span></div><p className="mt-2 text-sm">{staffName(ticket.sent_by)} · Enviado hace {elapsedFoodTime(ticket.sent_at, now)}</p>
              {checkItems.filter(item => item.ticket_id === ticket.id).map(item => <div className="mt-3" key={item.id}><div className="flex justify-between gap-2"><span>{item.quantity} × {item.product_name} · {item.variant_name}</span><span>{money(item.line_total)}</span></div><FoodItemModifiers modifiers={item.configuration?.modifiers} />{item.notes ? <p className="text-amber-500">{item.notes}</p> : null}{canVoid && !item.voided_at ? <div className="mt-2 flex gap-2"><button type="button" className={button} disabled={busy} onClick={() => void act("void_item", { checkId: check.id, itemId: item.id, reason: "Cancelación operativa", restock: false })}>Anular</button><button type="button" className={button} disabled={busy} onClick={() => void act("void_item", { checkId: check.id, itemId: item.id, reason: "Cancelación con reintegro", restock: true })}>Anular y reintegrar</button></div> : null}</div>)}
        ...
        {opening ? <PosModal open size="small" title={`Abrir ${opening === "COUNTER" ? "Mostrador" : opening === "TAKEAWAY" ? "Para llevar" : data.tables.find(table => table.id === opening)?.name || "mesa"}`} closeLabel="Cerrar apertura de mesa" dismissible={!busy} onClose={() => { if (!busy) setOpening(null); }} className="border border-[var(--pos-border)]">
          <form ref={openingForm} className="space-y-5" onSubmit={event => { event.preventDefault(); void act("open", opening === "COUNTER" || opening === "TAKEAWAY" ? { serviceType: opening, locationId: data.location?.id, guests: 1, customerName: customer } : { tableId: opening, guests, customerName: customer }); }}>
    ...
    {customizing && check && canEdit ? <PosFoodModifierDialog key={customizing.id} product={customizing} brandSlug={brandSlug} locationId={data.location?.id} checkId={check.id} variants={customizing.product_id ? data.products.filter(product => product.product_id === customizing.product_id) : [customizing]} visual={<ProductVisual imageUrl={customizing.image_url} category={customizing.category} />} money={money} busy={busy} error={error} onClose={() => { if (!busy) setCustomizing(null); }} onAdd={async (ids,notes,variantId) => {
      const added = await act("item_add", { checkId: check.id,variantId:variantId || customizing.id,quantity:1,notes,modifierOptionIds:ids });
