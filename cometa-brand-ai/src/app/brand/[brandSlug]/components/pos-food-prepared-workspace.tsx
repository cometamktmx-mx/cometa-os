        ...
        <header className="flex shrink-0 items-start justify-between gap-5 border-b border-white/10 bg-[#0b1c2a]/95 px-6 py-5 backdrop-blur-xl lg:px-8">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[.18em]">
              <span className="text-cyan-300">Preparado Food</span>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6 lg:px-8">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
            <main className="space-y-5">
              <Section eyebrow="01 · Producto" title="Información general" description="Presenta el producto como lo verá tu equipo y tu cliente.">
                <div className="grid gap-5 lg:grid-cols-[180px_minmax(0,1fr)]">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 rounded-xl border border-cyan-300/15 bg-cyan-300/[.05] px-4 py-3 text-sm"><span className="text-cyan-200">Tipo</span><strong>Preparado por receta</strong><span className="text-slate-500">·</span><span className="text-slate-400">La disponibilidad depende de sus insumos</span></div>
              ...
              <Section eyebrow="05 · Operación" title="Disponibilidad estimada" description="Conoce cuántas unidades puedes preparar con el inventario actual.">{recipe ? <Availability recipe={recipe} /> : <div className="rounded-xl border border-dashed border-white/15 p-5 text-sm text-slate-400">La disponibilidad aparecerá cuando publiques una receta válida.</div>}</Section>
            </main>
...
function Availability({ recipe }: { recipe: NonNullable<FoodRecipeProduct["recipe"]> }) { return <div className="space-y-3"><div className="grid gap-3 sm:grid-cols-2"><SummaryMetric label="Unidades posibles" value={recipe.availability == null ? "—" : `${recipe.availability}`} /><SummaryMetric label="Insumo limitante" value={recipe.limiting_ingredient || "—"} /></div><div className="rounded-xl border border-white/10 bg-white/[.025] p-4 text-sm"><p className="text-xs uppercase tracking-wider text-slate-500">Rendimiento por componente</p>{recipe.components.map(component => <div key={component.ingredient_variant_id} className="flex items-center justify-between border-b border-white/5 py-3 last:border-0"><span>{component.name}</span><strong>{component.possible} posibles</strong></div>)}</div><p className="text-sm text-slate-400">Remanente estimado: <strong className="text-slate-200">{recipe.limiting_remaining ?? "—"}</strong></p></div>; }
