...
type AnalyticsSummary = {
  ...
  sales: { grossSales: Metric; discountTotal: Metric; netSales: Metric; taxTotal: Metric; ordersCount: Metric; itemsSold: Metric; averageTicket: Metric; cogs: Metric; grossProfit: number; grossMarginPercent: number | null };
...
type OperationalVariant = { variantId: string; variantName: string; sku: string | null; attributes: Record<string, unknown> | null; attributesAreCurrent: boolean; unitsSold: number; revenue: number; cogs: number; grossProfit: number; grossMarginPercent: number | null; currentStock: number };
type OperationalProduct = { productId: string; productCode: string | null; productName: string; unitsSold: number; revenue: number; cogs: number; grossProfit: number; grossMarginPercent: number | null; currentStock: number; variants: OperationalVariant[]; productCodeIsCurrentMetadata: boolean };
type AnalyticsCustomer = { customerId: string; customerName: string; ordersCount: number; salesTotal: number; averageTicket: number };
type AnalyticsInventory = { productId: string; productName: string; variantId: string; variantName: string; sku?: string | null; locationId: string; currentQuantity: number; availableQuantity: number; minimumQuantity: number; unitCost?: number; inventoryCostValue: number; averageUnitsSoldPerDay?: number; daysOfStockEstimate: number | null };
...
type PeriodKey = "today" | "yesterday" | "7d" | "30d" | "month" | "previousMonth" | "custom";
...
export default function PosReportsPage() {
  ...
  const [operationalProducts, setOperationalProducts] = useState<BlockState<OperationalProduct[]>>(EMPTY);
  ...
  const [activeTab, setActiveTab] = useState<"summary" | "products" | "payments" | "inventory">("summary");
  ...
  const loadReports = useCallback(() => {
    ...
    load<OperationalProduct[]>("operational_products", setOperationalProducts);
      ...
      <div className="mx-auto max-w-[1580px] space-y-6">
        <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-col items-stretch gap-3 xl:items-end">
            ...
            <ReportsExportMenu brandSlug={brand.slug} range={range} locationId={filters.locationId} />
          </div>
        </header>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Kpi label="Ventas brutas" metric={summary.data?.sales.grossSales} value={formatMoney(summary.data?.sales.grossSales.current || 0)} loading={summary.loading} />
          <Kpi label="Descuentos" metric={summary.data?.sales.discountTotal} value={formatMoney(summary.data?.sales.discountTotal.current || 0)} loading={summary.loading} />
          <Kpi label="Impuestos" metric={summary.data?.sales.taxTotal} value={formatMoney(summary.data?.sales.taxTotal.current || 0)} loading={summary.loading} />
          <Kpi label="Tickets" metric={summary.data?.sales.ordersCount} value={integer.format(summary.data?.sales.ordersCount.current || 0)} loading={summary.loading} />
          ...
          <Kpi label="Unidades" metric={summary.data?.sales.itemsSold} value={integer.format(summary.data?.sales.itemsSold.current || 0)} loading={summary.loading} />
          <Kpi label="COGS" metric={summary.data?.sales.cogs} value={formatMoney(summary.data?.sales.cogs.current || 0)} loading={summary.loading} note="Costo histórico" />
          <Kpi label="Margen bruto" value={formatMoney(summary.data?.sales.grossProfit || 0)} loading={summary.loading} note="Ventas netas − impuestos − COGS" />
          <Kpi label="Margen %" value={summary.data?.sales.grossMarginPercent == null ? "—" : `${summary.data.sales.grossMarginPercent.toFixed(1)}%`} loading={summary.loading} note="Sobre ventas antes de impuestos" />
        </section>

        <nav aria-label="Secciones de reportes" className="flex max-w-full gap-2 overflow-x-auto rounded-2xl border border-white/[.08] bg-white/[.025] p-2">
          {([['summary','Resumen'],['products','Productos'],['payments','Pagos'],['inventory','Inventario']] as const).map(([key,label]) => <button key={key} type="button" onClick={() => setActiveTab(key)} className={`whitespace-nowrap rounded-xl px-4 py-2 text-xs font-black uppercase tracking-wider ${activeTab === key ? 'bg-cyan-300 text-slate-950' : 'text-slate-500 hover:bg-white/[.05] hover:text-white'}`}>{label}</button>)}
        </nav>
        ...
        {activeTab === "summary" ? <section className="grid gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
        ...
        {activeTab === "products" ? <Panel title="Productos y variantes" subtitle="Ventas históricas agrupadas por product_id; stock actual por variante" loading={operationalProducts.loading} error={operationalProducts.error}>
          <OperationalProductsTable rows={operationalProducts.data || []} formatMoney={formatMoney} />
        </Panel> : null}

        {activeTab === "payments" ? <Panel title="Métodos de pago" subtitle="Todos los pagos asociados al universo filtrado" loading={summary.loading} error={summary.error}>
          <PaymentsTable rows={summary.data?.payments || []} formatMoney={formatMoney} />
        </Panel> : null}

        {activeTab === "inventory" ? <Panel title="Inventario actual" subtitle="Valor estimado actual = disponible × costo actual" loading={inventory.loading} error={inventory.error}>
          <InventoryTable rows={inventory.data || []} locations={locations} formatMoney={formatMoney} />
        </Panel> : null}
...
function ReportsExportMenu({ brandSlug, range, locationId }: { brandSlug: string; range: ReturnType<typeof resolveRange>; locationId: string }) {
  const [open, setOpen] = useState(false);
  if (!range.valid) return null;
  const params = new URLSearchParams({ brandSlug, dateFrom: range.from.toISOString(), dateTo: range.to.toISOString() });
  if (locationId) params.set("locationId", locationId);
  const printHref = `/brand/${encodeURIComponent(brandSlug)}/pos/reports/print?${params.toString()}`;
  const csvHref = (type: string) => `/api/pos/reports/export?${new URLSearchParams({ ...Object.fromEntries(params), type }).toString()}`;
  return <div className="relative flex justify-end"><button type="button" onClick={() => setOpen((value) => !value)} className="rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-xs font-black uppercase tracking-wider text-cyan-100 hover:bg-cyan-300/20">Exportar</button>{open ? <div className="absolute right-0 top-11 z-20 w-56 rounded-2xl border border-white/10 bg-[#08111e] p-2 shadow-2xl"><a href={printHref} target="_blank" rel="noreferrer" className="block rounded-xl px-3 py-2 text-xs font-bold text-white hover:bg-white/[.06]">PDF Ejecutivo</a><a href={csvHref("products")} className="block rounded-xl px-3 py-2 text-xs font-bold text-slate-300 hover:bg-white/[.06]">CSV Productos</a><a href={csvHref("payments")} className="block rounded-xl px-3 py-2 text-xs font-bold text-slate-300 hover:bg-white/[.06]">CSV Pagos</a><a href={csvHref("inventory")} className="block rounded-xl px-3 py-2 text-xs font-bold text-slate-300 hover:bg-white/[.06]">CSV Inventario</a></div> : null}</div>;
}
...
function OperationalProductsTable({rows,formatMoney}:{rows:OperationalProduct[];formatMoney:(value:number)=>string}){if(!rows.length)return <Empty text="Sin productos vendidos en este periodo."/>;return <div className="space-y-3">{rows.map((row)=><details key={row.productId} className="rounded-2xl border border-white/[.07] bg-black/10" open={rows.length<=3}><summary className="cursor-pointer list-none p-4"><div className="grid gap-2 md:grid-cols-[minmax(0,1.5fr)_repeat(5,minmax(70px,1fr))] md:items-center"><div><p className="text-sm font-black text-white">{row.productName}</p><p className="text-[10px] font-bold text-slate-500">{row.productCode||"Sin clave"} · {row.productCodeIsCurrentMetadata?"metadato actual":""}</p></div><ReportNumber label="Unidades" value={integer.format(row.unitsSold)}/><ReportNumber label="Ventas" value={formatMoney(row.revenue)}/><ReportNumber label="Costo" value={formatMoney(row.cogs)}/><ReportNumber label="Margen" value={formatMoney(row.grossProfit)}/><ReportNumber label="Stock" value={integer.format(row.currentStock)}/></div></summary><div className="overflow-x-auto border-t border-white/[.06] p-3"><table className="w-full min-w-[760px] text-left text-xs"><thead className="text-[10px] uppercase tracking-wider text-slate-600"><tr><th className="p-2">Variante</th><th className="p-2">SKU</th><th className="p-2">Unidades</th><th className="p-2">Ventas</th><th className="p-2">Costo</th><th className="p-2">Margen</th><th className="p-2">Stock actual</th></tr></thead><tbody className="divide-y divide-white/[.05]">{row.variants.map((variant)=><tr key={variant.variantId}><td className="p-2 font-bold text-white">{variant.variantName}{variant.attributesAreCurrent?<span className="ml-2 text-[9px] font-normal text-slate-600">atributos actuales</span>:null}</td><td className="p-2 text-slate-400">{variant.sku||"—"}</td><td className="p-2 text-slate-300">{integer.format(variant.unitsSold)}</td><td className="p-2 text-cyan-200">{formatMoney(variant.revenue)}</td><td className="p-2 text-slate-300">{formatMoney(variant.cogs)}</td><td className="p-2 text-emerald-200">{formatMoney(variant.grossProfit)}</td><td className="p-2 text-slate-300">{integer.format(variant.currentStock)}</td></tr>)}</tbody></table></div></details>)}</div>}
function ReportNumber({label,value}:{label:string;value:string}){return <div><p className="text-[9px] font-black uppercase tracking-wider text-slate-600">{label}</p><p className="mt-1 text-sm font-black tabular-nums text-slate-200">{value}</p></div>}
function PaymentsTable({rows,formatMoney}:{rows:AnalyticsSummary["payments"];formatMoney:(value:number)=>string}){if(!rows.length)return <Empty text="Sin pagos registrados en este periodo."/>;return <div className="space-y-3">{rows.map((row)=><div key={row.paymentMethod} className="rounded-xl border border-white/[.06] bg-black/10 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-black text-white">{paymentLabel(row.paymentMethod)}</p><p className="text-[11px] text-slate-500">{integer.format(row.transactionsCount)} pagos</p></div><p className="text-lg font-black text-cyan-200">{formatMoney(row.amount)}</p><p className="text-xs font-black text-slate-400">{row.percentageOfSales==null?'—':`${row.percentageOfSales.toFixed(1)}%`}</p></div><div className="mt-3 h-1.5 rounded-full bg-white/[.06]"><div className="h-full rounded-full bg-cyan-300" style={{width:`${Math.min(row.percentageOfSales||0,100)}%`}}/></div></div>)}</div>}
...
function InventoryTable({rows,locations,formatMoney}:{rows:AnalyticsInventory[];locations:Location[];formatMoney:(value:number)=>string}){if(!rows.length)return <Empty text="Sin inventario para este alcance."/>;const names=new Map(locations.map(location=>[location.id,location.name]));return <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-xs"><thead className="text-[10px] uppercase tracking-wider text-slate-600"><tr><th className="p-2">Producto / variante</th><th className="p-2">SKU</th><th className="p-2">Sucursal</th><th className="p-2">Disponible</th><th className="p-2">Mínimo</th><th className="p-2">Costo actual</th><th className="p-2">Valor estimado</th><th className="p-2">Velocidad</th><th className="p-2">Días</th><th className="p-2">Estado</th></tr></thead><tbody className="divide-y divide-white/[.05]">{rows.map((row)=><tr key={`${row.variantId}-${row.locationId}`}><td className="p-2"><p className="font-black text-white">{row.productName}</p><p className="text-slate-500">{row.variantName}</p></td><td className="p-2 text-slate-400">{row.sku||"—"}</td><td className="p-2 text-slate-400">{names.get(row.locationId)||row.locationId}</td><td className="p-2 font-black text-white">{row.availableQuantity}</td><td className="p-2 text-slate-400">{row.minimumQuantity}</td><td className="p-2 text-slate-300">{formatMoney(row.unitCost||0)}</td><td className="p-2 text-cyan-200">{formatMoney(row.inventoryCostValue)}</td><td className="p-2 text-slate-300">{row.averageUnitsSoldPerDay==null?'—':row.averageUnitsSoldPerDay.toFixed(2)}/día</td><td className="p-2 text-slate-300">{row.daysOfStockEstimate==null?'—':row.daysOfStockEstimate.toFixed(1)}</td><td className={`p-2 font-black ${row.availableQuantity<=0?'text-rose-300':row.availableQuantity<=row.minimumQuantity?'text-amber-200':'text-emerald-300'}`}>{row.availableQuantity<=0?'Agotado':row.availableQuantity<=row.minimumQuantity?'Bajo stock':'Normal'}</td></tr>)}</tbody></table></div>}
...
function resolveRange(filters:ReportsFilters){const now=new Date();let from:Date,to=now;if(filters.period==='today'){from=new Date(now);from.setHours(0,0,0,0)}else if(filters.period==='yesterday'){to=new Date(now);to.setHours(0,0,0,0);from=new Date(to);from.setDate(from.getDate()-1)}else if(filters.period==='7d'){from=new Date(now.getTime()-7*864e5)}else if(filters.period==='30d'){from=new Date(now.getTime()-30*864e5)}else if(filters.period==='month'){from=new Date(now.getFullYear(),now.getMonth(),1)}else if(filters.period==='previousMonth'){to=new Date(now.getFullYear(),now.getMonth(),1);from=new Date(to.getFullYear(),to.getMonth()-1,1)}else{from=new Date(`${filters.customFrom}T00:00:00`);to=new Date(`${filters.customTo}T00:00:00`);to.setDate(to.getDate()+1)}return{from,to,valid:Number.isFinite(from.getTime())&&Number.isFinite(to.getTime())&&to>from}}
