...
type ProfileCode = "fashion" | "retail" | "restaurant" | "coffee_shop";
...
const POS_FOOD_PROFILE_CHOICES: Array<{
  code: Extract<ProfileCode, "restaurant" | "coffee_shop">;
  title: string;
  description: string;
  icon: PosIconName;
}> = [
  { code: "restaurant", title: "Restaurante", description: "Mesas, servicio, comandas y cocina.", icon: "receipt" },
  { code: "coffee_shop", title: "Cafetería", description: "Mostrador, barra, pedidos y clientes.", icon: "receipt" },
];
...
export default function BusinessOnboardingClient({ posFoodOnboardingEnabled }: { posFoodOnboardingEnabled: boolean }) {
        ...
        {loading ? (
          ...
          <form onSubmit={createBusiness} className="px-5 py-6 sm:px-9 sm:py-8">
            ...
            <fieldset className="mt-7">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {posFoodOnboardingEnabled
                  ? POS_FOOD_PROFILE_CHOICES.map((choice) => (
                      <ProfileChoice key={choice.code} {...choice} selected={profileCode === choice.code} onSelect={setProfileCode} />
                    ))
                  : POS_FOOD_PROFILE_CHOICES.map((choice) => (
                      <DisabledChoice key={choice.code} title={choice.title} icon={choice.icon} />
                    ))}
              </div>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
