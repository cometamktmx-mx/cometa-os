...
export default async function WorkspacePage() {
  ...
  if (!context.brands.length && context.hasPendingInvitation) redirect("/invite");
  if (!context.brands.length && context.profile?.status !== "inactive") redirect("/onboarding/business");
...
function EmptyClientWorkspace() { return <main className="flex min-h-screen items-center justify-center bg-[#050916] p-6 text-slate-100"><section className="max-w-lg rounded-3xl border border-white/10 bg-white/[.045] p-8 text-center"><p className="text-xs font-semibold uppercase tracking-[.24em] text-cyan-300">COMETA</p><h1 className="mt-4 text-3xl font-semibold">Aún no tienes una empresa en Cometa</h1><p className="mt-4 text-sm leading-6 text-slate-400">Crea tu negocio para comenzar.</p><Link href="/onboarding/business" className="mt-6 inline-flex rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950">Crear mi empresa</Link><p className="mt-5 text-xs leading-5 text-slate-500">¿Te invitaron a una empresa? <Link href="/invite" className="font-semibold text-cyan-300 hover:text-cyan-200">Revisa tu invitación.</Link></p></section></main>; }
