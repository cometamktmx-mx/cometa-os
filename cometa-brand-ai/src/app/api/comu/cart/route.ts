...
export async function POST(request: Request) { try { requireComuFeature("catalog"); const body = await request.json() as { listingId?: unknown; variantListingId?: unknown; quantity?: unknown; purchaseMode?: unknown; runId?: unknown; runCount?: unknown }; const input = { listingId: String(body.listingId || ""), variantListingId: String(body.variantListingId || ""), quantity: Number(body.quantity), ...(body.purchaseMode === "RUN" ? { purchaseMode: "RUN" as const, runId: typeof body.runId === "string" ? body.runId : null, runCount: Number(body.runCount || 1) } : {}) }; return cartResponse(await addCartItem(input)); } catch (error) { return fail(error); } }
export async function PATCH(request: Request) { try { requireComuFeature("catalog"); const body = await request.json() as { itemId?: unknown; quantity?: unknown; purchaseMode?: unknown; runId?: unknown; runCount?: unknown }; const mode = body.purchaseMode === "RUN" ? "RUN" as const : undefined; return cartResponse(await updateCartItem(String(body.itemId || ""), Number(body.quantity), mode, typeof body.runId === "string" ? body.runId : null, Number(body.runCount || 1))); } catch (error) { return fail(error); } }
