...
export default function AddToCart({ variants, listingId }: { variants: Array<{ id: string; name: string; available: boolean }>; listingId: string }) {
  ...
  async function add() {
    ...
    setBusy(true);
    try {
      const response = await fetch("/api/comu/cart", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ listingId, variantListingId, quantity }) });
