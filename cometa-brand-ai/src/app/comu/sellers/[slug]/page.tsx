  ...
  return <main className="mx-auto max-w-7xl px-5 py-8 md:py-12">
    <section className="overflow-hidden rounded-[2rem] border border-[#d9cfc4] bg-[#fffdf9] shadow-[0_24px_70px_rgba(72,55,40,.08)]">
      <div className="grid gap-8 p-6 md:grid-cols-[minmax(0,1fr)_minmax(19rem,.82fr)] md:p-10">
        <div className="flex min-w-0 flex-col justify-center">
          <div className="flex items-center gap-4">
            <div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-[#e2eadf] text-2xl font-black" style={{ color: accent }}>
              {logoUrl ? <img src={logoUrl} alt={`Logo de ${seller.public_name}`} className="h-full w-full object-cover" /> : seller.public_name.slice(0, 1).toUpperCase()}
            </div>
        ...
        <div className="relative aspect-[16/10] overflow-hidden rounded-[1.6rem] bg-[#252d28] md:aspect-auto md:min-h-72">{coverUrl ? <img src={coverUrl} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_18%,rgba(160,225,179,.32),transparent_34%),linear-gradient(135deg,#38463e,#b08f72)]" />}<div className="absolute inset-0 bg-gradient-to-t from-[#19201c]/55 via-transparent to-white/5" /></div>
      </div>
    </section>
    <section className="py-14"><div className="flex flex-wrap items-end justify-between gap-4"><SectionHeading eyebrow="Productos" title={`${listings.length} piezas para descubrir`} /><div className="flex gap-4 text-sm font-bold text-[#6d5b4e]"><span>Productos</span>{modes.includes("Mayoreo") ? <span>Mayoreo</span> : null}<span>Información</span></div></div>{listings.length ? <div className="mt-8 grid grid-cols-1 gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">{listings.map((listing) => <ProductCard key={listing.id} listing={listing} />)}</div> : <div className="mt-8"><EmptyState title="Esta tienda está preparando su catálogo." body="Las primeras piezas aparecerán cuando la selección esté lista para COMU." /></div>}</section>
  </main>;
}
